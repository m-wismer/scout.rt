...
export default class Table extends Widget {
  ...
  _updateTitleAggrCell($cell) {
    ...
    let cellWidth = this._getTitleAggrCellWidth($cell, $icon);
    cellWidth = this._updateTitleAggrIconVisibility($icon, $cellText, cellWidth);
   ...
   */
  _getTitleAggrCellWidth($cell, $icon) {
   ...
   */
  _updateTitleAggrIconVisibility($icon, $cellText, spaceAvailableForText) {
    if (!$icon || !$icon.length) {
